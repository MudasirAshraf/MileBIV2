...
const TableRelations = ({ datasets, dashboard, updateDashboard }) => {
    ...
    const tables = datasets.reduce((acc, dataset) => {
        acc[dataset.datasetTitle] = { data: dataset.dataSourceData, id: dataset.datasetId };
        return acc;
    }, {});
    ...
    const handleAddRelationship = async () => {
        if (!selectedMainTable || !selectedMainTableId || !selectedForeignKey ||
            !selectedRelatedTable || !selectedRelatedTableId || !selectedPrimaryKey) {
        ...
        const updatedDashboard = { ...dashboard, tableRelationships: newRelationships };
    ...
    const handleDeleteRelationship = async (index) => {
        ...
        const updatedDashboard = { ...dashboard, tableRelationships: updatedRelationships }
        await updateDashboard(updatedDashboard);
    };

    const optionsFromTable = (table) =>
        table ? Object.keys(tables[table].data[0] || {}).map((col) => ({ label: col, value: col })) : [];

    return (
        <div className="container mt-4">
            <Button onClick={() => setShowModal(true)}>Define Relationships</Button>
            ...
            <Modal show={showModal} size="xl" onHide={() => setShowModal(false)} centered>
                ...
                <Modal.Body>
                    <div className="form-group">
                        ...
                        <Select
                            options={Object.keys(tables).map((table) => ({ label: table, value: table, id: tables[table].id }))}
                            onChange={(option) => {
                        ...
                        {relationships.map((rel, index) => (
                            <Card key={index} className="p-3 mb-3">
                                <div className="d-flex align-items-center justify-content-between">
                                    <div>
                                        <strong>{rel.mainTable} (ID: {rel.mainTableId})</strong> ({rel.foreignKey})
                                        <FontAwesomeIcon icon={faArrowRight} className="mx-2" />
                                        <strong>{rel.relatedTable} (ID: {rel.relatedTableId})</strong> ({rel.primaryKey})
                                        {/* <div className="small text-muted">Fields: {rel.fields.join(", ")}</div> */}
                                    </div>
                                    <div>
                                        <Button variant="outline-danger" size="sm" onClick={() => handleDeleteRelationship(index)}>
