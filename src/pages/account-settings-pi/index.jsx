...
const AccountSettingsPI = ({ user,updateCurrentUser }) => {
  ...
  useEffect(() => {
    ...
    if (user) {
      axiosInstance.get(`/user/getbyid/${user.id}`)
        .then((response) => {
  ...
  const updateProfile = (values, setSubmitting) => {
    ...
    axiosInstance.put('/user/updateuser', values)
      .then((response) => {
      ...
      <div className='container-account-settings-pi'>
        <DashboardWrapper>
          <div className='account-settings-pi'>
            ...
            <p className='account-settings-ci-paragraph' onClick={handleAccountSettings}>
              Account Settings
            </p>
...
export default connect(mapStateToProps, { updateCurrentUser })(AccountSettingsPI);
